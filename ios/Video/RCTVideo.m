...
#if __has_include(<react-native-video/RCTVideoCache.h>) && !TARGET_OS_TV
  ...
  dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t) 0), dispatch_get_main_queue(), ^{
    ...
    [self playerItemForSource:source withCallback:^(AVPlayerItem * playerItem) {
      ...
      if (@available(iOS 10.0, tvOS 10.0, *)) {
        if (_repeat) {
          _queuePlayer = [AVQueuePlayer queuePlayerWithItems:@[_playerItem]];
          _playerLooper = [AVPlayerLooper playerLooperWithPlayer:_queuePlayer templateItem:_playerItem];
          _player = _queuePlayer;
        } else {
